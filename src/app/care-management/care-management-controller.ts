...
function buildExplanationPrompt(
  ...
  const citations = retrieveClinicalChunks(retrievalQuery, 5);
  ...
  const systemContent =
    ...
    (citations.length > 0
      ? '\n\nGround your explanation in the CLINICAL KNOWLEDGE block below. Cite sources in brackets like [PMID-12345678] or [MLP-J44.1].'
      : '');
...
export function createCareManagementController(mlModel: AlertAutoencoder) {
  ...
  return {
    ...
    async executeSLMExplanation(
      ...
      abortController = new AbortController();
      const messages = buildExplanationPrompt(state.coreVitals, state.uc2Result, state.selectedScenarioId ? undefined : 'default-patient');
