...
/**
 * Dynamically adjust runtime AE trigger threshold based on patient clinical risk tier
 * and GMFCS classification (e.g., GMFCS Level V or HIGH/CRITICAL risk tier).
 */
export function getAdjustedAEThreshold(
    ...
    // GMFCS Level V patients have higher baseline instability / severe impairment
    if (profile.gmfcs_level === "V" || profile.gmfcs_level === "Level V" || profile.gmfcs_level === "5") {
        multiplier *= 0.85; // lower threshold to increase sensitivity
    }

    if (profile.clinical_risk_tier === "CRITICAL") {
        multiplier *= 0.80;
    } else if (profile.clinical_risk_tier === "HIGH") {
        multiplier *= 0.90;
    } else if (profile.clinical_risk_tier === "LOW") {
        multiplier *= 1.10;
    }
