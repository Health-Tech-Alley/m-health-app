...
export interface AdherenceDerivationInput {
  /**
   * If true, derived adherence replaces any pre-existing log.adherence value.
   * If false, existing log.adherence is preserved and derivation is only used when missing.
   */
  overwriteExisting?: boolean;

  /**
   * Weight given to exercise completion ratio.
   * Default: 0.7
   */
  exerciseCompletionWeight?: number;

  /**
   * Weight given to therapy minutes completion ratio.
   * Default: 0.3
   */
  minutesCompletionWeight?: number;

  /**
   * If true, logs marked as medically skipped are excluded from adherence penalty.
   * Example: fever, clinician told patient to stop, urgent symptoms.
   */
...
/**
 * Derives a 0-1 adherence value from a caregiver daily rehab log.
 *
 * Priority:
 * 1. If log.adherence exists and overwriteExisting=false, keep it.
 * 2. If medically skipped and excused, return adherence=1.0 so patient is not penalized.
 * 3. Otherwise derive from:
 *    - completed exercises / assigned exercises
 *    - completed therapy minutes / planned therapy minutes
 *    - fallback sessionCompleted boolean
 */
