 ...
 *
 * Gemma 4 E2B-it (Q4_K_M, ~2.4 GB) remains the default Concierge model:
 * text-only clinical-decision + RAG role, smaller footprint, good llama.rn
 * fit, and a native `<|think|>` reasoning channel via jinja +
...
/**
 * Resolve the active Concierge model id: prefer the persisted default when it
 * is installed, otherwise the first installed catalog model, otherwise the
 * default id (surfaces should then show an install CTA).
 */
export function resolveActiveModelId(
  ...
  const fallback = MODEL_CATALOG.find((m) => isInstalled(m.id));
  return fallback?.id ?? DEFAULT_SLM_MODEL_ID;
}
