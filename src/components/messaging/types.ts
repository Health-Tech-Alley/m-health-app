...
export type MessagingIntegrationBoundary = {
  ...
  /**
   * Inject runtime context (patient ID + consent token) so the boundary
   * can stamp outbound messages and audit rows. The interface lets each
   * implementation decide whether to mutate state, return a new bound
   * instance, or treat the context as a constructor argument.
   */
  setContext?: (params: { patientId?: string; consentToken?: string }) => void;
};
